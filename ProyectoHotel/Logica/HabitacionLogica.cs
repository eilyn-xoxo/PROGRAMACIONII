        ...
        public bool Registrar(Habitacion oHabitacion)
        {
            ...
            using (SqlConnection oConexion = new SqlConnection(Conexion.CN))
            {
                try
                {
                    ...
                    cmd.Parameters.AddWithValue("IdEntrada", oHabitacion.oEntrada.IdEntrada);
        ...
        public List<Habitacion> Listar()
        {
            ...
            using (SqlConnection oConexion = new SqlConnection(Conexion.CN))
            {
                try
                {
                    ...
                    query.AppendLine("select h.IdHabitacion,h.Numero,h.Detalle,h.Precio,p.IdEntrada,p.Descripcion[DescripcionEntrada],c.IdCategoria,c.Descripcion[DescripcionCategoria],h.Estado,");
                    query.AppendLine("eh.IdEstadoHabitacion,eh.Descripcion[DescripcionEstadoHabitacion]");
                    query.AppendLine("from habitacion h");
                    query.AppendLine("inner join ENTRADA p on p.IdEntrada = h.IdEntrada");
                    ...
                    using (SqlDataReader dr = cmd.ExecuteReader())
                    {
                        while (dr.Read())
                        {
                            Lista.Add(new Habitacion()
                            {
                                ...
                                oEntrada = new Entrada() { IdEntrada = Convert.ToInt32(dr["IdEntrada"]), Descripcion = dr["DescripcionEntrada"].ToString() },
